...
class NotificationConsumer(AsyncJsonWebsocketConsumer):
    async def connect(self):
        if self.scope["user"].is_authenticated:
            ...
            print(f"""Added {self.channel_name} channel to alluser and {self.scope["user"].id}""")
    ...
    async def disconnect(self, close_code):
        ...
        print(f"""Removed {self.channel_name} channel to all-users and {self.scope["user"].id}""")
